...
class ControlConnection extends EventEmitter implements ConnectionInterface
{
	...
	protected function data(mixed $data): void {
		try {
			...
			foreach($responses as $data)
				...
				catch(\UnderflowException $e) { throw $e; } // Global error.
				catch(\Throwable $e) { $this->emit('error', [$e]); } // Command error.
		}
		catch(\Throwable $e) {
	...
	protected function handle(ModelInterface $message): void {
		$this->queue->isEmpty() and throw new \UnderflowException('Unexpected reply received, no matching request found (ENOMSG)',
